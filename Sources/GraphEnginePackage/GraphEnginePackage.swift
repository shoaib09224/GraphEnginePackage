...
public class GraphEnginePlugin {
    ...
    public func fetchCountriesDetails(url: String, headers: [String:String]? = nil, countryCode: String,completion: @escaping((GetCountryInfoQuery.Data?, [GraphQLError]?) -> Void)) {
        Network.makeRequest(url: url, headers: headers).fetch(query: GetCountryInfoQuery(code: countryCode)) { result in
            switch result {
            case .success(let graphQLResult):
                print("Success! Result: \(String(describing: graphQLResult.data))")
                completion(graphQLResult.data, nil)
